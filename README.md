...
-- [name=Jonathan Rasmusson / Former Spotify Engineer, The Agile Samurai]
- Extract-Down：由上而下檢查，讓最大值跑到最上面
    - 由上往下比較主節點與子節點大小，比到子節點的值大於主節點的值就交換，一路往同個分支的左右兩邊比，沒有更大的就維持原本的排序。
- 把最後一個非空值的位置跟最大值交換，並且固定此最大值的位置在最後忽略不計，再從最頂端的主節點進行Extract-Down，重覆到只剩下最後一個元素時停止。
- Insert-Up：如果需要插入，把值插入最後一個，再一路跟主節點比大小，遇到比自己小的就交換位置，值到無法交換停止。
